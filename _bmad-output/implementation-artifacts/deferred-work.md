...
- **已解决（2026-07-22）：** `run_amc_attribution.py` 已通过 `write_csv_set_atomic` 整组发布五份模型结果。
...
- 独立运行 `run_amc_attribution.py` 时应拒绝 AMC 聚合路径输入与任一正式输出目标为同一路径，避免错误参数覆盖输入；完整 `run_pipeline.py` 已有对应保护。
- 路径构建当前允许触点时间恰好等于购买时间；应明确业务契约是允许零时间差互动，还是要求触点严格早于购买，再单独调整代码和边界测试。
...
- **已被取代（2026-07-21）：** 当前 14/13/14 契约不再输出稳定性、决策或自动化字段；未来如需此类治理，应使用独立产物。
- **已取代（2026-07-20）：** 原高/中/低多级可靠性方案及稳定性、跨 outcome 组合不再作为可靠性标准。当前只使用计算有效、最低数据支持、模型一致三个布尔值及二元 `RELIABLE/UNRELIABLE` 状态。
- **已被取代（2026-07-21）：** 当前输出只保留 `gap_pp`、`relative_gap` 与 `models_consistent`，不再输出差距等级。
