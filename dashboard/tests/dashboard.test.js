...
const SETTINGS_DIALOG = readFileSync(
  resolve(HERE, "..", "src", "views", "Settings.vue"),
  "utf8",
);
 ...
 * copy is imported through a data URL with the build flag substituted and those
 * helpers exported. Its page-registry import is absolute; no helper is written
 * into maintained source directories.
 *
...
async function loadClientModule(staticBuild = false) {
  ...
  const source = CLIENT.replaceAll("import.meta.env", `{ VITE_STATIC_BUILD: "${flag}" }`)
    .replace('"../pages.js"', JSON.stringify(new URL("../src/pages.js", import.meta.url).href)) +
    "\nexport { readResourceStream, windowStaticPayload };\n";
  clientModules.set(flag, await import(`data:text/javascript;base64,${Buffer.from(source).toString("base64")}`));
...
test("Settings is a routed page and Reload lives only inside it", () => {
  ...
  assert.equal(PAGES.settings.defaultSection, "general");
  ...
  assert.doesNotMatch(SIDEBAR_NAV, /emit\(['"]settings/);
  assert.match(SETTINGS_DIALOG, />Reload data</);
  assert.match(SETTINGS_DIALOG, /class="settings-page card"/);
  assert.doesNotMatch(SETTINGS_DIALOG, /aria-label="Settings"[^>]*role="dialog"/);
});
...
test("settings compares independently detected frontend and backend builds", () => {
  ...
  assert.match(SETTINGS_DIALOG, /const tab = computed\(\(\) => props\.section\)/);
  assert.match(SETTINGS_DIALOG, /emit\("navigate", nextTab\)/);
...
test("a static host honours the same window against its whole files", async () => {
  ...
  const longHistory = { simulationResearch: {
    history: [{ report_date: "2025-01-01" }, { report_date: "2026-01-01" }, { report_date: "2026-03-31" }],
    delivery: [], historyWindow: { earliest: "2025-01-01", latest: "2026-03-31" },
  } };
  const quarter = windowStaticPayload(longHistory, { start: "2026-02-30" });
  assert.equal(quarter.simulationResearch.historyWindow.start, "2026-01-01");
  assert.equal(quarter.simulationResearch.history.length, 2);
  const reversed = windowStaticPayload(longHistory, { start: "2026-03-31", end: "2026-01-01" });
  assert.deepEqual(reversed.simulationResearch.history, quarter.simulationResearch.history);
  const whole = {
    simulationResearch: {
...
test("route resources load lazily with immediate backend phase progress", async () => {
  ...
  assert.deepEqual(parseRoute("#/settings/tasks"), { page: "settings", section: "tasks" });
