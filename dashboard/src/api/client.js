...
function windowStaticPayload(payload, window) {
  ...
  let start = calendarDate(window?.start);
  let end = calendarDate(window?.end);
  if (start && end && start > end) [start, end] = [end, start];
  if (!start && !end) {
    const earliest = calendarDate(research.historyWindow.earliest);
    const latest = calendarDate(research.historyWindow.latest);
    if (earliest && latest) {
      const first = new Date(new Date(`${latest}T00:00:00Z`).getTime() - 89 * 86400000).toISOString().slice(0, 10);
      if (first > earliest) { start = first; end = latest; }
    }
  }
  const within = (rows) =>
    (rows ?? []).filter((row) => {
