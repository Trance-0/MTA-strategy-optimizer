...
├── zh/                               # 完整中文源文档，暂不发布
├── assets/                           # 共享图像与图表
├── research/                         # 外部 PDF、DOCX、JSON、TXT 原件
...
模块内原有策略说明已迁移到项目级文档目录 `docs/en/strategy_recommendation/`，包括
...
`docs/zh/strategy_recommendation/`，供未来中文站点使用；其中优化计划仍是未实现的后续研究。
...
新增或移动文件必须遵循[工作区文件位置管理](file-management.md)。双语多文件栏目统一用
`index.md` 作为入口；历史模型功能说明现位于
