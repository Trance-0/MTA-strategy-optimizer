# 文档索引

当前项目事实、全工作区结构、阅读顺序和文档状态统一从
[全工作区文档总索引](index.md)进入。模块参数、字段契约和命令已归入
当前发布入口统一使用 `en/` 下的 VitePress 栏目；`zh/` 保留中文源文件但暂不发布；
`modules/` 只保留代码、数据和可再生产物。

## 当前事实

- [当前文档总索引](index.md)
- [工作区总览与现状评价](project-overview.md)
- [工作区级架构](architecture.md)
- [用户维护的系统架构图源文件](../../系统架构图-07.drawio)
- [目录结构分析](source-tree-analysis.md)
- [工作区文件位置管理](file-management.md)
- [组件与资产清单](component-inventory.md)
- [开发与验证指南](development-guide.md)
- [全量文件清单](../../workspace-file-inventory.json)
- [AMC MTA 工作区文档](../product/amc-mta/)
- [AMC MTA 架构](../product/amc-mta/architecture.md)
- [AMC MTA 能力评价](../product/amc-mta/capability-assessment.md)
- [AMC MTA 项目介绍](../product/amc-mta/project-introduction.md)
- [Campaign Group 顶层数据关系](../research/campaign-data-hierarchy.md)
- [MTA Strategy Initializer](../strategy/module-overview.md)
- [项目扫描状态](../../project-scan-report.json)

## 契约与运行

- [模块说明](../attribution/amc-mta-module.md)
- [数据契约](../datasets/amc-data-contract.md)
- [运行说明](../environment/amc-mta-usage.md)
- [模型比较治理](../attribution/model-governance.md)
- [触点可靠性指南](../attribution/reliability.md)
- [初始策略模型计划](../strategy/model-plan.md)
- [当前 Ad Group 初始预算计算详解](../strategy/current-budget-calculation.md)
- [MTA 到 Ad Group 预算问题定义与研究计划](../strategy/optimization-plan.md)

## 历史与研究

- [产品文档状态](../product/)
- 历史设计产物：`design-artifacts/README.md`
- [研究资料分级索引](../research/)
- [Campaign Group 顶层数据关系与最细效果粒度](../research/campaign-data-hierarchy.md)

历史愿景和研究快照用于追溯，不覆盖当前模块的代码、数据契约和治理规范。
