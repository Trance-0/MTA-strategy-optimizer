...
- 策略初始化说明与研究已迁移到项目级 `docs/en/strategy_recommendation/`，中文源文件保存在
  `docs/zh/strategy_recommendation/`。当前实现以 `model-plan.md`、两份输出契约和
...
- 研究分级与原件索引：两个语言树中的 `research/`；外部二进制原件保留在 `docs/research/`。
