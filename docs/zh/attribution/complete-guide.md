...
默认演示输入位于 `modules/amc_mta/data/simulated/`，文件角色见[模拟数据说明](../datasets/amc-simulated-data.md)：
...
完整字段、数值关系和对齐条件以[数据契约](../datasets/amc-data-contract.md)为准。
...
详细规则见[可靠性判断](reliability.md)和
[双模型治理规范](model-governance.md)。
...
上一批派生产物保持不变。自定义路径和分步命令见[运行方式](../environment/amc-mta-usage.md)。

独立验证模块测试时，不运行流水线，从仓库根目录执行：
...
1. 从 [AMC MTA 模块说明](amc-mta-module.md)了解范围和自动窗口。
2. 展示[数据契约](../datasets/amc-data-contract.md)中的五段键和 14 天规则。
3. 运行完整流程与对齐校验。
4. 按[输出索引](output-reference.md)顺序查看五份结果。
5. 用推荐表解释 `official`、`RELIABLE` 和不可靠区间的边界。
6. 用[提交清单](../reference/submission-manifest.md)确认核心包与辅助材料边界。
