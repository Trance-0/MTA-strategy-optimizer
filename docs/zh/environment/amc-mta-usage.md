...
以下所有命令均从仓库根目录运行。字段与路径规则见[数据契约](../datasets/amc-data-contract.md)。
...
```bash
python3 -B modules/amc_mta/scripts/run_amc_attribution.py
```

严格复算已有 Markov/Shapley 文件的三份比较产物：

```bash
python3 -B modules/amc_mta/scripts/compare_attribution_models.py
```
...
输入位于 `modules/amc_mta/data/simulated/`；文件角色见 [AMC MTA 模拟数据](../datasets/amc-simulated-data.md)。
...
[单触点归因可靠性判断说明](../attribution/reliability.md)。
