...
- Added source-preserving Campaign optimizer links, automatic ordinary-history fitting, configurable similarity grouping and numeric thresholds with algorithm tooltips; verified 198 backend tests, 179 dashboard tests, documentation build and the PostgreSQL Docker browser preview, including explicit refusals for unavailable historical evidence.
...
- Rewrote the Campaign budget optimizer documentation in 0.9.57 so an agent can rebuild the implementation from it. The old single page named the classes but withheld what a reproduction needs — the search grids behind the fit, both bisections in the shadow-price solver, the artifact's tolerances — so it became five pages answering, in order, what model, what assumptions, how the data is collected for prediction, what data model, how it is implemented, where each intermediate product is saved, and what comes out; every figure in them is read from the committed artifact or from the source. Removed the heading numbers across the section, which had stopped describing anything once pages were split — `current-budget-calculation/` ran one sequence across four files and `current-implementation.md` carried two `### 1.`–`### 8.` runs at once — and replaced the ordering they implied with `order` frontmatter the sidebar actually sorts on. Corrected three claims the old page made: the test count is 116, the artifacts are committed deliberately rather than never, and the two callers that build the Initial Strategy disagree, which the backend's docstring denies; the last is recorded as known drift because choosing a side is a code change. Verified with 768 Python tests, 176 dashboard tests, and a documentation build with no dead link.

- Added the local-preview rule to `AGENTS.md` in 0.9.56: a change that alters a major component or adds a feature must ask the owner whether to bring up the existing `deploy/docker/` stack first, with database credentials taken from the ignored root `.env` rather than any tracked file. The automated suites all run in file mode, so passing them says nothing about the database path a deployed instance uses, and the deployment step is too late to find that out. Published the rule set itself as [Repository Rules Index](../en/introduction/repository-rules.md) — 107 rules, one sentence each, numbered continuously, with the introduction date and author of each read from `git log` rather than reconstructed. It is a documentation page rather than an appendix to `AGENTS.md` because the two serve different readers; a person reviewing the rules has less context to spend than the agent that loads the whole file. Verified by script that both files carry the same eighteen sections in the same order with matching counts and no gap in the numbering.
...
- Repaired the container-publishing backend gate for its deliberate source-only checkout: only the real MTA-SIM toy integration case now skips without the pinned submodule, while configuration-boundary and route tests stay hermetic. Missing simulator source is reported as bounded capability state, and known preset requests return `503 generator_unavailable` without leaking the runner path instead of raising an unhandled 500. Removed schema setup's second deployed-runtime assumption by launching initialization and derivation through the backend's active Python interpreter, so a missing `uv` executable cannot stop either operation before Python starts. Added a write-probed default pipeline runtime and fixed backend file-mode execution; added exact-manifest model output upload/download, backend parsing, explicit PostgreSQL artifact import, and restart restoration. Moved deployment identity into a basic General tab and enabled bounded structured INFO logging with severity/source filters, copy, clear, and optional durations following Notechondria's public guide.
- Delivered the submodule content to Gitea, which cannot reach GitHub and so could never resolve a mirrored gitlink into files. The mirror publishes an additional `gitea-snapshot` branch whose tree has the submodule content materialized as ordinary tracked files and no `.gitmodules`, exempt from pruning and from the exact-match reference check while every other branch and tag still has to match GitHub exactly. Found that `external/campaign-optimizer-llm-integration` declares this repository as one of its own submodules, so the obvious recursive fix re-enters the project and retrieves a stale copy of itself; initialization is one level deep everywhere, and the six documented `--recursive` instructions that would have triggered the cycle were corrected, including the adapter's own error message. Closed the gap that made the work invisible: `external` was excluded from both images, so the Data Generator would have reported the pinned simulator missing on every deployment. The generator package and its presets now ship through a single-match glob that keeps the copy optional, verified by building both images — with the submodule the generator reports itself available, without it the same image reports bounded unavailability instead of failing. 104 backend and 99 `mta_standard` tests pass and the documentation site builds with no dead link.
...
- Diagnosed the remote service through direct no-proxy requests, exposed its five-schema census, split the 51.7-megabyte Campaign history from the core snapshot, and wrote the Alembic expand-contract migration and continuous-deployment plan. Runtime schema selection now confirms, probes, and rolls back safely; a failed schema offers backend-declared load, derive, or initialize controls without replacement, including on a credential-protected deployment.
- Added Data Generator after Command Center with guided and JSON editors, bounded asynchronous calls into the pinned MTA-SIM package, two 20-row previews, two declared CSV downloads, and backend-only PostgreSQL export with write-only credentials and trusted-proxy-aware HTTPS enforcement. Browser verification generated the toy tables, loaded actual `mta_us` data, rendered Campaigns, and found no console error.
- Completed server-issued model datasets and native Willow execution, flattened the seven-view navigation, moved reload into Settings, removed backend-unintegrated Knowledge Base content, and added accessible key-term definitions linked to precise English documentation. The complete change set passes 100 backend and 45 dashboard tests plus production client and documentation builds.
