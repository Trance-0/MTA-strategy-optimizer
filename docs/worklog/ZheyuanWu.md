...
- Reviewed and committed a prior working session's deploy-bundle restructuring: a single `run.sh`, uploaded beside `.env`, now embeds `enqueue_deploy.sh` and `deploy_worker.sh` as checksummed, `bash -n`-verified payloads and creates one `deploy/installation/{config,app,state,systemd}` tree in place of the previous fixed `/etc`, `/opt`, and `/var/lib` paths, with ACL-scoped traversal grants and legacy-path migration once the new installation is healthy. Caught a real consequence during that review before committing: the embedded `deploy_worker.sh` payload was a byte snapshot still carrying the pre-fix `dashboard_healthy()` from earlier today, so the single-file install path would have silently redeployed the exact health-check bug just fixed above; regenerated the payload and its checksum against the current file and confirmed a byte-for-byte match. From the same prior session, also reviewed and committed a pipeline architecture diagram for the English docs homepage hero: a new theme `Layout.vue` fills VitePress's `#home-hero-image` slot with the existing `DrawioDiagram` component on that page only, with CSS overrides replacing the slot's circular-logo styling with a full-width flow diagram.
