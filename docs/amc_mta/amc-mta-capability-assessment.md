...
| 维度 | 等级 | 证据 |
| --- | --- | --- |
| 工程可靠性 | 较高 | 流程可复现、校验严格，106 项测试覆盖守恒、可靠性与回滚 |
...
收入相差 6.930 个百分点。精简结果保留该触点的 `gap_pp`、`relative_gap` 和
`models_consistent`，但不再输出差距等级或关键分歧标签；正式使用前仍应优先解释
该路径结构。
