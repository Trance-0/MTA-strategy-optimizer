...
| `candidates` | Eligible candidate counts per Campaign, as in the current [`candidate_pool.json`](/en/strategy/output-data-contract) |
| `entity_bridge` | Historical touchpoint-to-entity relationships for bridging attribution grain to Campaign grain |
| `lineage` | Content hashes and scope of the evidence files, matching the pattern in the [current initializer](/en/strategy/module-overview#1-verify-evidence-lineage-before-calculation) |
...
`CampaignAllocation` mirrors the current [Ad Group output contract](/en/strategy/strategy-output-contract#4-ad-group-output):
...
- [Strategy output contract](/en/strategy/strategy-output-contract)
- [Ad Group initial-budget output data contract](/en/strategy/output-data-contract)
