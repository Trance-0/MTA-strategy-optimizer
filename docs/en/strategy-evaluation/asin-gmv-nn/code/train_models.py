...
仅 numpy / matplotlib。无昨日惯性、无岭回归、无 asin one-hot。
"""
...
def design_matrix(rows: list[dict], catalogs: dict[str, list[str]] | None = None):
    ...
    x = np.hstack([numeric, dow, country])
    y = {
...
def main() -> None:
    ...
    report = {
        ...
        "hidden": list(HIDDEN),
        "inputs": list(BUDGET_COLS) + list(SHARE_COLS) + ["has_ad", "is_weekend", "dow", "marketplace"],
        "label": "SK-II attributed sales (no ASIN)",
        "product": "SK-II",
        "baselines_removed": ["persistence_lag1", "ridge_log_elastic", "asin"],
        ...
        "caveat": "训练含 Jan–Jun 模拟日；测试段含真实 July 后半。无 ASIN。",
    }
