...
- Efficiency: `roas` ([Return on Ad Spend](/en/reference/definitions#roas-return-on-ad-spend)), `roi` ([Return on Investment](/en/reference/definitions#roi-return-on-investment)), `cpa` ([Cost Per Acquisition](/en/reference/definitions#cpa-cost-per-acquisition--cost-per-action)), `cost_per_converted_user`.
