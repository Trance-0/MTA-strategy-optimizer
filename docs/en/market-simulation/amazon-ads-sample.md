...
`INTERACTION_TYPE` may only be `IMPRESSION` or `CLICK`. Run `uv run python -X utf8 -B -m modules.mta_attribution.src.validate_data_alignment` from the repository root to validate the account, currency, window, five-segment touchpoint set, and daily coverage.
