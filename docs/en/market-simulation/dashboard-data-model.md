...
One of them constrains this schema directly. **Row order is part of the contract**, because the views render in the order the loader returns, and a file loader returns the artifact's own order. Every table therefore carries a surrogate `id` primary key, `backend/import_to_database.py` inserts rows in each artifact's order, and every query the dashboard issues orders by that key rather than by the business key — `order by campaign_id` sorts alphabetically and would put the same Campaigns on screen in a different sequence than file mode does.
