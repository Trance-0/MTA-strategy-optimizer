---
title: Documentation Index
lang: en-US
---

# Documentation Index

Use the [complete workspace documentation index](index.md) as the single entry point for current project facts, workspace structure, reading order, and document status. Module parameters, field contracts, and commands are organized in the relevant VitePress sections. `en/` is the active published site; `zh/` preserves the Chinese source documents but is not currently published. `modules/` contains only code, data, and reproducible artifacts.

## Current Facts

- [Complete workspace documentation index](index.md)
- [Workspace overview and current assessment](project-overview.md)
- [Workspace architecture](architecture.md)
- [Current workspace architecture and co-located Draw.io source](architecture.md)
- [Source-tree analysis](source-tree-analysis.md)
- [Workspace file-location management](file-management.md)
- [Component and asset inventory](component-inventory.md)
- [Development and verification guide](development-guide.md)
- [Development history](development-history.md)
- [Project-level implementation specification catalog](../introduction/specifications.md)
- [Full machine-generated file inventory](../../workspace-file-inventory.json)
- [AMC MTA workspace documentation](../product/amc-mta/)
- [AMC MTA architecture](../product/amc-mta/architecture.md)
- [AMC MTA capability assessment](../product/amc-mta/capability-assessment.md)
- [AMC MTA project introduction](../product/amc-mta/project-introduction.md)
- [Campaign Group hierarchy](../research/campaign-data-hierarchy.md)
- [MTA Strategy Initializer](../strategy/module-overview.md)
- [Project scan status](../../project-scan-report.json)

## Contracts and Execution

- [AMC MTA module](../attribution/amc-mta-module.md)
- [Data contract](../datasets/amc-data-contract.md)
- [Execution guide](../environment/amc-mta-usage.md)
- [Model-comparison governance](../attribution/model-governance.md)
- [Touchpoint reliability guide](../attribution/reliability.md)
- [Initial strategy model plan](../strategy/model-plan.md)
- [Current Ad Group initial-budget calculation](../strategy/current-budget-calculation.md)
- [MTA-to-Ad-Group budget problem definition and research plan](../strategy/optimization-plan.md)

## History and Research

- [Product-document status](../product/)
- [English development history](development-history.md)
- [Implementation specification catalog](../introduction/specifications.md)
- Historical design artifacts: `design-artifacts/README.md`
- [Research-material classification index](../research/)
- [Campaign Group hierarchy and finest performance grain](../research/campaign-data-hierarchy.md)

Historical visions and research snapshots support traceability; they do not override current module code, data contracts, or governance rules.
