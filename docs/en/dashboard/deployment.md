...
compact: "Dashboard build integration in dashboard/build, backend static export, docs/.vitepress Pages assembly, Docker inputs and generated-data exclusions; canonical fixture provenance, build identity and publishing without root script/ helpers."
lang: en-US
source_files: .github/workflows/deploy-pages.yml, deploy/docker/nginx.conf, dashboard/index.html, dashboard/package.json, dashboard/vite.config.js, dashboard/run.sh, dashboard/run.bat, deploy/docker/compose.yaml, deploy/docker/defaults.env, deploy/docker/run.sh, deploy/docker/run.bat, deploy/docker/Dockerfile.api, deploy/docker/Dockerfile.dashboard, .dockerignore, .github/workflows/publish-containers.yml, docs/.vitepress/build_pages_site.mjs, backend/export_dashboard_snapshot.py, dashboard/build/import_ontology_review_fixtures.mjs, dashboard/tests/ontology_review_fixtures.test.js
---
...
`backend/export_dashboard_snapshot.py` forces local-file mode and writes the
...
`docs/.vitepress/build_pages_site.mjs` verifies it again after copying to final
...
site, then `docs/.vitepress/build_pages_site.mjs` assembles the dashboard at `/` and the
...
Specification ownership is reviewed from page metadata and source contracts
before publication. The retired one-off `spec_docs.py` helper is not a build
dependency. The product verification workflow still runs the product suites
and builds both client targets and the documentation.

Docker contexts exclude `dashboard/public/data/`. It is generated by the fixture
importer or static exporter, never a release input. Each image prepares its
own verified fixture directory in the build layer; copying a local generated
directory into a lower image layer can make atomic replacement fail with
`EXDEV` on an overlay filesystem. Both client images copy the tracked importer,
canonical source fixtures, and Willow model before building.

The September 5 host job failed before the frontend build because its
external generator checkout was incomplete. Follow the
[preflight and host recovery procedure](/en/introduction/backend/deployment-preflight)
before rerunning the job. The repository verification workflow initializes the
pinned generator and runs that check explicitly.
...
Source: `.dockerignore`, `deploy/docker/compose.yaml`, `deploy/docker/defaults.env`,
...
- Inputs: The repository root as build context; the dashboard sources; the
  tracked Willow Extended-27 model JSON imported by the client; `defaults.env`;
  the optional root `.env`; the optional `deploy/docker/.env` for ports; and
  `IMAGE_NAMESPACE` for the registry owner, defaulting to `trance-0`.
...
- Behavior contract: `run.sh up` builds and `run.sh pull` fetches, and both
  ...
  Ontology Review importer through `COPY dashboard/ ./`, including `dashboard/build/`; that copy
  ...
  by service name.
  `Dockerfile.dashboard` copies the one Willow model JSON to the same repository-
  relative location the Vue import resolves locally. `.dockerignore` excludes
  the rest of `docs/` while admitting that file and its parent directories, so
  the container build has every declared module input without absorbing the
  documentation and research corpus.
...
- Verification: `./deploy/docker/run.sh up`, then both containers reach
  ...
  `/api/dashboard` returns fifteen keys; and the eight pages render in a browser
...
Source: `.github/workflows/publish-containers.yml`, `.github/workflows/deploy-pages.yml`
...
### `backend/export_dashboard_snapshot.py`

Source: `backend/export_dashboard_snapshot.py`
...
- Verification: `uv run --extra backend python -X utf8 -B -m backend.export_dashboard_snapshot`;
  assert the result reports `mode: local files` and fourteen keys.

### Canonical Ontology Review build inputs and verification

Source: `dashboard/package.json`, `dashboard/build/import_ontology_review_fixtures.mjs`, `dashboard/tests/ontology_review_fixtures.test.js`
...
### `docs/.vitepress/build_pages_site.mjs`

Source: `docs/.vitepress/build_pages_site.mjs`
