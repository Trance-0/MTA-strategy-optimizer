...
compact: "Backend service map for `backend/`: Flask owns runtime data access, serves the Vue client contract, runs three model jobs including strategy evaluation, and exposes synchronous attribution, recommendation, full-history Campaign optimization and baseline references and attribution-evaluation endpoints. Routes share JSON error rules."
lang: en-US
...
- Inputs: The fields specified on the three endpoint configuration pages, including
  campaignId/marketplace/datasetId/historyMode for source-isolated own or full
  comparable history, baseline matching and observed-reference fallback;
