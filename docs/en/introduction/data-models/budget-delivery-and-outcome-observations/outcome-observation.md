...
This is the profit-relevant class in the canonical model: a future strategy optimizer's profit objective would need `incremental_units` — not `total_units` — multiplied by [Product Economics](/en/introduction/data-models/product-identity-and-economics/product-economics.md)'s `unit_contribution_margin` and compared against observed cost, since crediting advertising with organic demand it did not cause would overstate its profitability. No incrementality-estimation source or optimizer exists yet; this class only reserves the shape that objective would read.
