...
`None` when the report is not scoped to a specific campaign group. Unlike Touchpoint's optional fields, this has no companion `FieldAvailability`, ReportingScope predates and does not use the [Field Availability](./field-availability.md) pattern for this field.
...
Implemented and tested in `modules/mta_common/tests/test_budget_and_delivery.py`'s `ReportingScopeTests` (end date before start date rejected, blank required fields rejected) and exercised by `modules/mta_common/tests/test_legacy_adapters.py`'s campaign-group adaptation tests. See [Canonical Data Model](./index.md) for the full test count and command.
