...
Defined in `modules/mta_common/src/attribution_evidence.py`. Sits in the "historical evidence" layer of the [Canonical Data Model](./index.md): built from a [Touchpoint](./touchpoint.md) and a [Reporting Scope](./reporting-scope.md), and consumed, in the future, by [Campaign Episode](./campaign-episode.md) as one of the observed-after-treatment records a response model would read.
...
Not applicable; `AttributionEvidence` has no missingness states of its own for this field, unlike [Touchpoint](./touchpoint.md)'s optional fields. A blank or whitespace-only value is rejected outright, not represented as a missing state.
...
Every `AttributionEvidence` describes exactly one [Touchpoint](./touchpoint.md), embedded by value rather than referenced by an identifier, so the evidence record is self-contained.

### Relationship to Reporting Scope

Every `AttributionEvidence` carries the [Reporting Scope](./reporting-scope.md) it was computed over, rather than inlining `marketplace`/`advertiser_id`/`currency`/window fields of its own.

### Relationship to Campaign Episode

A future [Campaign Episode](./campaign-episode.md) would compose one campaign's `AttributionEvidence` records among its observed-after-treatment fields, alongside [Budget Observation](./budget-observation.md), [Delivery Observation](./delivery-observation.md), and [Outcome Observation](./outcome-observation.md). This is not implemented for `AttributionEvidence` specifically yet — see Downstream Usage.
...
Both adapters route `row.touchpoint`/`result.touchpoint` through `touchpoint_from_five_segment_key` or `touchpoint_from_four_segment_key` (see [Touchpoint](./touchpoint.md#legacy-mapping)) before constructing the `AttributionEvidence`.
...
A future response-prediction model would read `AttributionEvidence` as one of several observed-after-treatment inputs composed into a [Campaign Episode](./campaign-episode.md). A future strategy optimizer would read `AttributionEvidence` alongside [Product Economics](./product-economics.md) and future incrementality evidence, but only through whatever composition a not-yet-built optimizer defines — `AttributionEvidence` itself never becomes an optimization target by growing a new field. Neither consumer is implemented yet.
