...
- `rationale` and `generated_by` use plain `str | None`, not the five-state `FieldAvailability` vocabulary the core model uses elsewhere (see [Field Availability](./field-availability.md)) — a deliberate simplification, since this is a presentation-only type with no requirement to distinguish *why* an optional display field is absent.
