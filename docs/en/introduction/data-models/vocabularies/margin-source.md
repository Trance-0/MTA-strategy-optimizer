...
[Product Economics](/en/introduction/data-models/product-identity-and-economics/product-economics.md)'s `margin_source: MarginSource | None = None` field gates `unit_contribution_margin`. Its docstring: "`unit_contribution_margin` may be given directly (`margin_source=EXPLICIT`) or derived from `unit_price` and `unit_cogs` (`margin_source=DERIVED`). When both an explicit margin and its components are present, they must agree." `ProductEconomics` also keeps `unit_cogs` unset rather than zero-filled when unknown, since "zero and unknown are not the same claim" — the same missingness discipline [Field Availability](/en/introduction/data-models/vocabularies/field-availability.md) applies elsewhere in this module, expressed here as a plain optional field rather than a five-state enum.
