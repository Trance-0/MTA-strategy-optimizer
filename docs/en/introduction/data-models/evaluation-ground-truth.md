...
No class described elsewhere in this documentation section may hold a field shaped like this one. [Campaign Episode](./campaign-episode.md) is checked for exactly that by `evaluation_only.assert_no_ground_truth_fields`.
...
- `EvaluationGroundTruth` is never accepted as a field on any class described elsewhere in this documentation section. It is accepted only by [Evaluation Episode](./evaluation-episode.md), which exists specifically to pair it with a `CampaignEpisode` outside the model-facing type itself.

## Relationships <span class="status-label status-verified" aria-label="Verified"></span>

### Relationship to Evaluation Episode

[Evaluation Episode](./evaluation-episode.md) is the only class that holds an `EvaluationGroundTruth` field. It pairs one `EvaluationGroundTruth` with one `CampaignEpisode`, by composition.

### Relationship to Campaign Episode

[Campaign Episode](./campaign-episode.md) has no field of this type, and cannot be given one without changing its definition — this is what `assert_no_ground_truth_fields(CampaignEpisode)` verifies. See that page's Invariants section.
...
Only a simulator-backed evaluation harness should ever construct this — see [Evaluation Episode](./evaluation-episode.md) for how it is combined with model-facing data without leaking into it.
