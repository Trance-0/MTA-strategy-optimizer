...
The advertising platform this touchpoint was delivered on, independent of `ad_product`. Two touchpoints can share the same `ad_product` string while naming different providers; see [Provider](/en/introduction/data-models/vocabularies/provider.md)'s Examples.
...
The provider-specific advertising product, for example `SPONSORED_PRODUCTS`. Validated against a [Provider Capabilities](/en/introduction/data-models/touchpoint-and-provider-contract/provider-capabilities.md)'s `supported_ad_products` by callers that hold one; `Touchpoint` itself does not require a `ProviderCapabilities` to construct.
...
[Touchpoint Field Availability](/en/introduction/data-models/touchpoint-and-provider-contract/touchpoint-field-availability.md).
...
Cross-validated field-by-field against `placement`/`creative`/`interaction_type` as described above; not separately validated as a standalone object beyond its own `__post_init__` (it has none — see [Touchpoint Field Availability](/en/introduction/data-models/touchpoint-and-provider-contract/touchpoint-field-availability.md)).
...
`Touchpoint.provider` is typed as [Provider](/en/introduction/data-models/vocabularies/provider.md). `Touchpoint.field_availability` is a [Touchpoint Field Availability](/en/introduction/data-models/touchpoint-and-provider-contract/touchpoint-field-availability.md), whose three fields are each typed as [Field Availability](/en/introduction/data-models/vocabularies/field-availability.md).

### Relationship to Provider Capabilities

A [Provider Capabilities](/en/introduction/data-models/touchpoint-and-provider-contract/provider-capabilities.md) instance is the static ceiling a caller may validate a `Touchpoint`'s `ad_product` and `field_availability` against; `Touchpoint` does not hold or require one directly.

### Relationship to Delivery Observation, Outcome Observation, and Attribution Evidence

[Delivery Observation](/en/introduction/data-models/budget-delivery-and-outcome-observations/delivery-observation.md), [Outcome Observation](/en/introduction/data-models/budget-delivery-and-outcome-observations/outcome-observation.md), and [Attribution Evidence](/en/introduction/data-models/historical-evidence-and-lineage/attribution-evidence.md) each carry a `touchpoint: Touchpoint` field identifying what was observed or evaluated.
...
Implemented and tested in `modules/mta_common/tests/test_touchpoint.py` (provider/ad_product separation, `NOT_APPLICABLE`/`NOT_PROVIDED` distinguishability, value/availability consistency enforcement, required-field rejection) and `modules/mta_common/tests/test_legacy_adapters.py` (five-segment round-trip, four-segment key never guessing `interaction_type`). See [Canonical Data Model](/en/introduction/data-models/index.md) for the full test count and command.
