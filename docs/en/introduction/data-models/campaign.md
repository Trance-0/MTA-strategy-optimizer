...
Defined in `modules/mta_common/src/campaign.py`, alongside [Ad Group](./ad-group.md). This is the Campaign Identity layer of the [Canonical Data Model](./index.md): it depends on [Provider](./provider.md) and [Reporting Scope](./reporting-scope.md), and is in turn depended on by [Campaign Episode](./campaign-episode.md), which composes a `Campaign` with its budget, delivery, and outcome observations.
...
Not applicable. `Campaign` carries no optional-field machinery — every field on this class must always carry a value. The five explicit [Field Availability](./field-availability.md) states describe why a `Touchpoint` component may be missing, not why a campaign identity field would be; a record that cannot supply a `campaign_id` is not a valid `Campaign`.
...
Provider-specific advertising product, for example `SPONSORED_PRODUCTS`. `Campaign` itself imposes no closed vocabulary; a caller that needs to reject an unrecognized product validates the value against a [Provider Capabilities](./provider-capabilities.md) instance for `provider`, not against `Campaign`.
...
- `provider` and `ad_product` are independent — no rule ties one to a fixed set of the other's values inside this class. Enforcing that a given `provider` only issues certain `ad_product` values is a caller responsibility, exercised through [Provider Capabilities](./provider-capabilities.md), not a `Campaign` invariant.
...
[`AdGroup`](./ad-group.md) belongs to exactly one `Campaign` through `AdGroup.campaign_id`, a plain string reference rather than a database-enforced foreign key — `Campaign` does not hold a collection of its ad groups.

### Relationship to Reporting Scope

Every `Campaign` embeds one [`ReportingScope`](./reporting-scope.md) directly as a field, rather than referencing one by identifier.

### Relationship to Campaign Episode

[`CampaignEpisode`](./campaign-episode.md) composes a `Campaign` with its [`BudgetConstraints`](./budget-constraints.md), [`BudgetObservation`](./budget-observation.md), [`DeliveryObservation`](./delivery-observation.md), [`OutcomeObservation`](./outcome-observation.md), and [`AttributionEvidence`](./attribution-evidence.md) records into the shape a future response model would consume.

### Relationship to Campaign Product Link

A future [`CampaignProductLink`](./campaign-product-link.md) references a campaign by `campaign_id`, expressing the many-to-many relationship between campaigns and [`Product`](./product.md) that `Campaign` itself does not carry.
...
A future strategy optimizer would read a campaign's [`CampaignEpisode`](./campaign-episode.md) — which embeds this `Campaign` — to decide a budget allocation, and a future response model would use `provider`/`ad_product` as categorical features. Neither consumer exists yet.
...
- `Campaign` does not itself enforce that `ad_product` belongs to `provider`'s real vocabulary; that check is deferred to a caller consulting [Provider Capabilities](./provider-capabilities.md), which this class does not reference.
