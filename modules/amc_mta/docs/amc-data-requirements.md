...
所有 CSV 入口都对字段名和值使用 `strip()` 语义清除首尾空白；字符串内部空格
原样保留，不删除也不折叠。清理后的表头必须非空且唯一，两个原始字段若清理后
同名会立即报错。每一行的列数必须与表头完全一致，多列或少列都不会被补齐、
丢弃或静默接受。

AMC 路径和模型结果等严格入口在完成上述规范化后，仍要求字段顺序和完整 schema
...
- 每个 `CONVERSION` 必须位于自动识别的 Ads 窗口内；窗口外转化会终止整次运行，
  不会被裁剪。原始触点可以早于窗口，用于执行既有的起点边界判断。
...
Ads 日期必须从最早日至最晚日连续，且每天具有完全相同的五段触点集合；同一
`reportDate + normalizedTouchpoint` 不得重复。程序不会根据事件日期缩短窗口，
也不会对 Ads 日期缺口静默补零。
...
比较输入的表头在清理首尾空白后必须精确等于完整契约；两个模型的触点集合、成本和平台表现完全一致，
