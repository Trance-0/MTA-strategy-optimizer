...
| `amazon_ads_report_sample.csv` | 同期 Amazon Ads 风格五段触点表现和成本；这是归因输入，不承担 Campaign Group 管理层级 |
...
五段键是归因观察维度，不是业务实体树。`AD_PRODUCT` 在该键中继续保留，不表示它位于 Campaign 与 Ad Group 之间。`Campaign Group → Campaign → Ad Group → Keyword/SKU` 的独立模拟数据见 [`../../../mta_strategy_recommender/data/simulated/`](../../../mta_strategy_recommender/data/simulated/)。
