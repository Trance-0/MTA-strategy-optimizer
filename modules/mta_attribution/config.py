"""Fixed paths and report constants for the attribution module.

Single source of truth for where the simulated inputs live, where published
artifacts are written, and the report window and privacy thresholds the sample
dataset was generated under.

Data flow: imported by `run_pipeline` and every script so that a default path
appears in exactly one place. Callers may override any of these with an explicit
CLI argument; nothing here is read at import time by the models themselves.
"""
