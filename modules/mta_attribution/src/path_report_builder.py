"""Reconstruct anonymous aggregated paths from journey events.

Turns per-user event streams into the aggregated path contract, applying the
windowing rules that decide which touchpoints belong to which conversion.

Rules enforced here: a path starts strictly after the report start, ends with a
purchase inside the report window, every adjacent step including the final
touchpoint-to-purchase gap is within `max_gap_days`, and a prior purchase splits
a journey into segments that cannot be reused.

Data flow: touchpoint event rows -> validated events -> per-conversion path
segments -> aggregated rows keyed by (marketplace, advertiser, path).
"""
