"""The canonical touchpoint key and its component rules.

Owns the single definition of what a touchpoint key is, so no other file has to
restate the rules. A key has five colon-separated components:

    AD_PRODUCT:FORMAT:PLACEMENT:CREATIVE:INTERACTION_TYPE

Components are uppercase ASCII letters, digits, and underscores. Placement and
creative fall back to `UNSPECIFIED` when built from columns, but never when
parsed from an existing key, where a blank component is a contract violation.

Data flow: every layer that reads, writes, or compares a touchpoint calls in
here, which is what makes path rows and Ads rows joinable at all. The format
column is product-specific: `inventoryType` for `AMAZON_DSP`, `adType` for
`SPONSORED_*`.
"""
