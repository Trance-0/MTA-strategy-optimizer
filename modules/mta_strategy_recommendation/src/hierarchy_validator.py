"""Validate the strategy inputs and their evidence lineage.

Checks that the request, candidate pool, attribution evidence, and entity bridge
describe one consistent Campaign Group before any budget is calculated, and pins
the evidence by SHA-256 so a recommendation can always be traced to the exact
attribution artifact it came from.

Data flow: strategy request + candidate pool + attribution CSV + bridge CSV
-> a validated summary, or a `HierarchyValidationError` naming the first
inconsistency. It also rejects forbidden output content, keeping the strategy
boundary to budget only.
"""
