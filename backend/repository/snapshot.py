...
def parse_history_window(start: Any, end: Any) -> dict[str, str | None]:
    ...
    Malformed bounds are dropped and a reversed valid pair is swapped. The
    browser resolver applies the default 90-day window when neither survives;
    direct loaders and the static exporter can still request unbounded history.
    """
    first = str(start).strip() if start not in (None, "") else None
    last = str(end).strip() if end not in (None, "") else None
    def calendar_bound(value):
        if not value or not _DATE_PATTERN.fullmatch(value):
            return None
        try:
    ...
    first = calendar_bound(first)
    last = calendar_bound(last)
